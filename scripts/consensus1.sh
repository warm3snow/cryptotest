...
# 生成节点证书
SUBJ="/C=CN/ST=BeiJing/L=BeiJing/O=wx-org1.chainmaker.org/OU=common/CN=consensus1.sign.wx-org1.chainmaker.org"
openssl ecparam -out consensus1.sign.key -name prime256v1 -genkey -noout
openssl req -key consensus1.sign.key -new -out consensus1.sign.csr -subj $SUBJ -config server.openssl.cnf
openssl x509 -req -in consensus1.sign.csr -CA ca.crt -CAkey ca.key -out consensus1.sign.crt -days 0 -extfile custom_server.openssl.cnf

#SUBJ="/C=CN/ST=BeiJing/L=BeiJing/O=wx-org5.chainmaker.org/OU=common/CN=consensus1.tls.wx-org5.chainmaker.org"
#openssl ecparam -out consensus1.tls.key -name prime256v1 -genkey -noout
#openssl req -key consensus1.tls.key -new -out consensus1.tls.csr -subj $SUBJ -config server.openssl.cnf
#openssl x509 -req -in consensus1.tls.csr -CA ca.crt -CAkey ca.key -out consensus1.tls.crt -days 3650 -extfile custom_server.openssl.cnf
#
#rm -rf consensus1.sign.csr consensus1.tls.csr
#
#
## 生成管理员证书
